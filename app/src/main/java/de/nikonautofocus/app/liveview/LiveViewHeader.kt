 ...
 *
 * Field offsets follow the layout digiCamControl uses for Nikon bodies. Note that unlike
 * PTP itself, this header is **big endian**.
 *
 *   offset  type  field
 *   0       u16   LiveView image width  (width of the JPEG that follows)
 *   2       u16   LiveView image height
 *   4       u16   whole image width     (coordinate space of the AF frame)
 ...
 *
 * The header length itself varies per body and firmware (8, 128 and 384 byte variants
 * exist), which is why the JPEG is located by marker scan and the header size is whatever
 * precedes it. The offsets above only hold for the long variants, so everything parsed
 * here is validated against the decoded JPEG before it is trusted - see [parse].
 */
...
object LiveViewHeaderParser {
    ...
    /** Smallest header that can still contain the movie recording byte at offset 60. */
    private const val MIN_PARSABLE_HEADER = 62
     ...
     * @return the parsed header, or null when it does not look like the documented layout.
     *
     ...
     * that the width/height stored at offset 0/2 must match the JPEG that follows.
     */
    fun parse(
        data: ByteArray,
        headerSize: Int,
        ...
        if (headerSize < MIN_PARSABLE_HEADER || headerSize > data.size) return null

        val liveViewWidth = u16(data, 0)
        val liveViewHeight = u16(data, 2)

        // The header must describe the image it is attached to.
        if (liveViewWidth != jpegWidth || liveViewHeight != jpegHeight) return null

        val imageWidth = u16(data, 4)
        val imageHeight = u16(data, 6)
        if (imageWidth <= 0 || imageHeight <= 0) return null

        val focusFrameWidth = u16(data, 16)
        val focusFrameHeight = u16(data, 18)
        val focusX = u16(data, 20)
        val focusY = u16(data, 22)
        ...
        val rotationByte = data[29].toInt() and 0xFF
        val rotation = when (rotationByte) {
            1 -> -90
            2 -> 90
            else -> 0
        }
        ...
        return LiveViewHeader(
            ...
            focused = (data[40].toInt() and 0xFF) != 1,
            rotationDegrees = rotation,
            movieRecording = (data[60].toInt() and 0xFF) == 1
        )
    }
